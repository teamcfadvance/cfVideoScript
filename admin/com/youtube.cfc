<!---
	...
	History      : Added getVideo, parseEntry support, getEmbedCode
				   Added delete and update methods. (Update 21/05/08 by Roland Keijzer)
--->
...
<cffunction name="getComments" access="public" returnType="query" output="false"
	...
	<cfargument name="videoid" type="string" required="true">
	<cfset var commentsurl = "#arguments.videoid#">
	<cfset var result = "">
...
<cffunction name="getMostLinkedVideos" access="public" returnType="query" output="false"
			hint="Gets the most linked videos.">
	<cfargument name="time" type="string" default="" hint="Restrict to: today,this_week,this_month,all_time">								
	<cfargument name="start" type="numeric" required="false">
	<cfargument name="max" type="numeric" required="false" hint="Defaults to 25 in API, max is 50">
	<cfset var baseurl = variables.standardurl & "most_linked?">
	<cfif len(arguments.time)>
		<cfset baseurl = baseurl & "&time=#arguments.time#">
	</cfif>
	<cfif structKeyExists(arguments, "start")>
...
<cffunction name="getThumbnail" access="public" output="false" returntype="struct" hint="I get the url paths for the you tube video sent in">
<cfargument name="videoId" type="string" required="true" hint="Video id to use">
	<cfset var youTubeUrl = "http://img.youtube.com/vi/">
	<cfset var thumbData = structNew()>
	<cfset thumbData['thumb1'] = youTubeUrl & ListLast(arguments.videoId,'videos/') & "/1.jpg">
	<cfset thumbData['thumb2'] = youTubeUrl & ListLast(arguments.videoId,'videos/') & "/2.jpg">
	<cfset thumbData['thumb3'] = youTubeUrl & ListLast(arguments.videoId,'videos/') & "/3.jpg">
	<cfset thumbData['thumbWidth'] = "140">
	<cfset thumbData['thumbHeight'] = "106">
	<cfset thumbData['full'] = youTubeUrl & ListLast(arguments.videoId,'videos/') & "/0.jpg">
...
<cffunction name="login" access="public" returnType="void" output="false"
	...
	<cfset var theurl = "https://www.google.com/youtube/accounts/ClientLogin">
	<cfset var result = "">
	<cfset var lines = "">
	...
	<cfif not listFirst(result,"=") is "auth">
		<cfthrow message="YouTubeCFC Login Error: #result#">
	</cfif>

	<cfset lines = listToArray(result, chr(10))>
	<cfset variables.authtoken = listRest(lines[1],"=")>
	<cfset variables.username = listRest(lines[2],"=")>
