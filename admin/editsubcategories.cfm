<cfif listlast(cgi.script_name, "/") eq "editsubcategories.cfm">
...
<cfif isDefined('url.searchcatname')>
  <cfinvoke component="#request.cfcPath#.video" method="getSubCats" searchcatname="#url.searchcatname#" set="1" returnvariable="Recordset1"/>
  <cfelseif isDefined('url.l')>
  <cfinvoke component="#request.cfcPath#.video" method="getSubCats" l="#url.l#" returnvariable="Recordset1"/>
  <cfelse>
  <cfinvoke component="#request.cfcPath#.video" method="getSubCats" sortorder="#sort#" returnvariable="Recordset1"/>
</cfif>
...
<cfif isDefined('url.delete')>
  <cfinvoke component="#request.cfcpath#.video" method="deleteSubCategory" SubcatID="#trim(url.delete)#"/>
  <cfset n = "Cool! Sub-Category Deleted Successfully">
  <cflocation addtoken="no" url="index.cfm?action=editsubcategories&n=#ToBase64(n)#">
</cfif>

<cfif isDefined('url.mode')>
  <cfinvoke component="#request.cfcpath#.video" method="updatesubCatActiveness" 
  mode="#url.mode#" subcatID="#trim(url.value)#"/>
  <cfset n = "Cool! Sub-Category Status updated Successfully">
  <cflocation addtoken="no" url="index.cfm?action=editsubcategories&n=#ToBase64(n)#">
</cfif>
<table align="center" width="100%">
  <tr align="center" class="oncaption">
    <td><a href="index.cfm?action=subcats">Create Sub-Category</a></td>
  </tr>
  <tr align="left">
  ...
  <tr align="left">
    <td align="center"><label>Find Sub-Category:</label>
      <input type="text" name="searchcatname" id="searchcatname" />
      &nbsp;
      <input type="submit" name="action" value="Find" /></td>
  </tr>
  </cfform>
</table>
<br />
<div align="center"> <cfoutput>Records #StartRow_Recordset1# to #EndRow_Recordset1# of #Recordset1.RecordCount# </cfoutput> </div>
<div align="left"> <cfoutput><a href="index.cfm?action=editsubcategories&sort=1">Sort by A-Z</a> | <a href="index.cfm?action=editsubcategories&sort=2">Sort by Z-A</a></cfoutput> </div>
<div align="center"> <cfoutput>
    ...
    <Cfloop from="1" to="#ListLen(alpha)#" index="L">
      ...
      <a href="index.cfm?action=editsubcategories&l=#listgetat(alpha, L)#">#listgetat(alpha, L)#</span></a>
    </CFLOOP>
    | <a href="index.cfm?action=editsubcategories"><strong>All</strong></a> </cfoutput> </div>
<table width="100%" border="0" align="center">
  ...
  <cfoutput query="Recordset1" startRow="#StartRow_Recordset1#" maxRows="#MaxRows_Recordset1#">
    ...
    onmouseout="this.className='rowEven'"</cfif>">
      <td>#Recordset1.SubCatDesc#</td>
      <td>#MainCat#</td>
      <td><a href="index.cfm?action=editsubcategories&mode=#URLEncodedFormat(status)#&value=#subcatID#">
	  <cfif #status# IS 1>Active<cfelse>Inactive</cfif></a></td>
      <td><a href="index.cfm?action=editsubcategories&delete=#URLEncodedFormat(subCatID)#" onClick="return confirm('Are you Sure.');">Delete</a> | <a href="index.cfm?action=subcats&edit=#URLEncodedFormat(subCatID)#">Edit</a></td>
    </tr>
  </cfoutput>
  <cfelse>
